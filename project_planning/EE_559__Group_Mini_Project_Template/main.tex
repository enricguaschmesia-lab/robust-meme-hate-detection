...
attainable (PGD AUROC $0\!\rightarrow\!0.62$, stable to strong attacks) but
trades $\approx\!0.12$ clean AUROC, with a combined model robust to both threats.
...
\textbf{Robustness-aware training.} For this part of training, both encoders are frozen and we are just training on the fusion head. Each batch pairs a clean view with an
...
\textbf{White-box attacks.} Here the image encoder is unfrozen as this proved to have a positive impact on training. We probe the worst case with image-space
...
$<\!0.01$ (ASR $>0.98$) by $\epsilon{=}2/255$, and to $0$ by $\epsilon\geq4/255$
(Table~\ref{tab:model_robustness}). Adversarial training --- unfreezing the
vision encoder, which the pixel attack targets --- restores robustness but
...
\vskip -0.08in
\caption{Cross-robustness on \texttt{test\_unseen} ($3$-seed mean
...
some clean accuracy can be spent; otherwise \texttt{KLDrop} --- high clean and
robust to the frequent naturalistic threat --- is the practical default.
