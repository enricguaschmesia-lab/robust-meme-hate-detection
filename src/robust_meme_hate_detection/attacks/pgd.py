...
Both attacks support ``norm='linf'`` (default) and ``norm='l2'``. The new
``norm`` keyword defaults to ``'linf'`` so existing callers (e.g.
``eval/run_whitebox.py``) keep their previous behaviour unchanged.
"""
